...
def getAccounts(pSelection):
    ...
    if globalVariables.CACHE_ACCS["newData"]:
        CurrentHeader = []
        CurrentTable = []
        CurrentHeader.append(html.Th("Účet"))
        for a in pSelection:
                ...
                for b in pSelection:
                    if b in currentAcc.keys():
                        if globalVariables.columHeaders[b]["type"] == "":
                            ...
                            tdText = HTMLUtils.strHTML(
                                HTMLUtils.reformatLogTime(currentAcc[b], "%H:%M:%S", True))
...
def getAccountHTML(pAccount, pLogAmount):
    if not pAccount == "":
        ...
        if secondsDiff > 40:
            logUtil.log(2,"Last r was "+str(secondsDiff)+" s ago")
            rCache=[html.H2(
                "Poslední načtení dat proběhlo před "+HTMLUtils.strHTML(secondsDiff)+"s Načítám data.."
                )]
            rCache.extend(globalVariables.ACCOUNTS_o[pAccount].logData)
            return rCache
        if globalVariables.ACCOUNTS_o[pAccount].newDataL:
            rCache = []
            rCache.append(html.H3("Logy pro účet: " + pAccount))
            LogData = [html.Tr([
                html.Th("Čas"),
                html.Th("Typ"),
                html.Th("Zdroj"),
...
def threadUpdateAccs():
    while True:
        ...
        if dif < 40:
            logUtil.log(1,"Last Call for getAccounts " + str(dif) + " s ago, going to update Acc Overview")
            updateAccs()
...
def threadUpdateAccountLog():
    while True:
        ...
        for a in globalVariables.ACCOUNTS_o:
            ...
            if dif < 40:
                logUtil.log(1,"Last Call for getAccLog (Account: " + a + ") " +
