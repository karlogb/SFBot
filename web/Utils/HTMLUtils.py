...
def reformatLogTime(pTime, pFormat="%H:%M:%S", pTimeLeft=False):
    ...
    if pTimeLeft:
        seconds = (dt_object-datetime.now()).total_seconds()

        if seconds > 0:
            return "{} ({}m)".format(dt_object.strftime(pFormat), floor(seconds/60))
